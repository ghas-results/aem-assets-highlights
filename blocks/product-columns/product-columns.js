export default async function decorate(block, window) {
  ...
  const columns = [...ctasRow.children].map((cta, i) => {
    ...
    const variantChooser = [...imageCell.lastElementChild.children]
      .map(({ href: href2, title, textContent }) => {
        const childContent = textContent.trim();
        const iconUrl = new window.URL(`${window.hlx.codeBasePath}/icons/circle_color_${childContent}.jpg`, window.location.href);
        return `
                    <a href="${href2}" title="${title}">
